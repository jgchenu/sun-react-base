...
function Tooltip(props: TooltipProps) {
  ...
  const bindTriggerEvents = useMemo(() => {
    if (!disabled) {
      ...
      if (isHover && children) {
        const originMouseEnter = children.props['onMouseEnter'];
        const originMouseLeave = children.props['onMouseLeave'];
        return {
  ...
  const isClassComponent =
    ...
    childrenType.prototype.isReactComponent;
  const safetyChildren = isClassComponent ? (
    props.children
  ) : (
