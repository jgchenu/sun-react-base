...
export default function (opts: IGetRollupConfigOpts): RollupOptions[] {
  const { type, entry, cwd, rootPath, importLibToEs, bundleOpts, sourcemap } =
    opts;
  const {
    umd,
    esm,
  ...
  try {
    ...
    console.error('require pkg error', (e as Error).message);
  }
  ...
  function testExternal(pkgs: string[], excludes: string[], id: string) {
  ...
  switch (type) {
    case 'esm': {
      const esmObj = isObject(esm) ? esm : {};
      const output: Record<string, string> = {
        dir: join(cwd, `${esmObj.dir || 'dist'}`),
        entryFileNames: `${esmObj.file || `${name}.esm`}.js`,
      };

      const sourcemapOption = sourcemap
        ? {
            sourcemap: sourcemap,
            sourcemapFile: `${name}.${type}.js`,
          }
        : {};
      ...
      return [
        {
          ...
          plugins: [
            ...getPlugins(),
            ...(esmObj.minify ? [terser(terserOpts)] : []),
          ],
          ...
          ? [
              {
                ...
                output: {
                  format,
                  file: join(cwd, `dist/${esmObj.file || `${name}`}.mjs`),
                  ...sourcemapOption,
                },
                plugins: [
                  ...getPlugins(),
      ...
      return [
        {
          ...
          output: {
            format,
            file: join(cwd, `dist/${cjsObj.file || name}.js`),
            ...sourcemapOption,
          },
          plugins: [
            ...getPlugins(),
            ...(cjsObj.minify ? [terser(terserOpts)] : []),
          ],
          external: testExternal.bind(null, external, externalsExclude),
        },
      ];
